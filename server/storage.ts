...
const defaultData: Database = { chatSessions: {} };
const db = new Low(adapter, defaultData);

export async function saveChatMessage(deviceId: string, content: string) {
  await db.read();
  
  if (!db.data.chatSessions[deviceId]) {
    db.data.chatSessions[deviceId] = [];
  }
  
  db.data.chatSessions[deviceId].push({
    id: uuidv4(),
    deviceId,
    content,
...
export async function getChatHistory(deviceId: string): Promise<ChatMessage[]> {
  await db.read();
  return db.data.chatSessions[deviceId] || [];
}
